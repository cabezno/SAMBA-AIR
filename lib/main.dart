...
class _HomePageState extends State<_HomePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _loadSaved();
    _log('SAMBA AIR v0.5.0 iniciado — por BLACKMAGICBOX');
  }
  ...
  static Color colorFor(Transport t) => switch (t) {
    ...
    Transport.sbl  => const Color(0xFFFF5E5E),
  };
  ...
  Future<void> _manualEntry() async {
    final items = <String>['WHIP (WebRTC)', 'SRT', 'RTMP', 'SBL 4K (UDP)'];
    String selected = _transportLabel == 'SBL 4K' ? 'SBL 4K (UDP)' : _transportLabel;
    ...
    Transport t;
    if (proto.contains('SBL')) {
      final parts = url.split(':');
      final ip = parts[0];
      final port = parts.length > 1 ? int.tryParse(parts[1]) ?? 8890 : 8890;
      cfg = ConnectionConfig.fromSblIp(ip, port: port);
      t   = Transport.sbl;
    } else if (proto.contains('SRT')) {
      final parts = url.split(':');
      final ip = parts[0];
  ...
  String _urlLabel(String proto) {
    if (proto.contains('SBL'))  return 'IP:puerto (ej. 192.168.1.2:8890)';
  ...
  String _urlHint(String proto) {
    if (proto.contains('SBL'))  return '192.168.1.2:8890';
  ...
  Future<void> _connectSbl(ConnectionConfig cfg) async {
    ...
    try {
      await sbl.connectTo(sblCfg.host, port: sblCfg.port);
      setState(() { _live = true; });
  ...
  Future<void> _toggleTorch() async {
    ...
    switch (_transport) {
      ...
      case Transport.srt:
      case Transport.sbl:
        // SRT/SBL uses native plugin — torch via channel
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          // Brand Logo — rotates when _connecting is true
          Center(
            child: Container(
              ...
              child: AnimatedBuilder(
                ...
                builder: (context, child) {
                  return Transform.rotate(
                    angle: _connecting ? _rotationCtrl.value * 2 * 3.1415926535 : 0,
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(70),
                      child: Image.asset(
                        'assets/samba_logo.png',
                        fit: BoxFit.cover,
                      ),
                    ),
                  );
  ...
  Widget _buildTransportLegend() => Column(
    ...
    children: const [
      Divider(),
      SizedBox(height: 4),
      _LegendRow(color: Color(0xFFFF5E5E), label: 'SBL 4K', desc: 'LAN UDP, Ingestor 4K, MLO, ultra-baja latencia'),
  ...
  Widget _buildLiveView() {
    ...
    switch (_transport) {
      ...
      case Transport.sbl:
        ...
        bitrate = sbl.bitrateMbps;
        latency = sbl.latencyMs;
        onAir   = sbl.isOnAir;
    }
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: Stack(fit: StackFit.expand, children: [
          ...
          // Active Logo Badge (samba_active.png / thumb.png)
          Positioned(
            top: 48,
            left: onAir ? 115 : 16,
            child: Container(
