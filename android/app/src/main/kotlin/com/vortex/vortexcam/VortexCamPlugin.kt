...
// =============================================================================
// VortexCamPlugin — unified native plugin for SRT + RTMP streaming
//
// Pipeline (SRT and RTMP):
...
class VortexCamPlugin(
    ...
    private var sblSocket:  DatagramSocket? = null // SBL transport
    ...
    // ====================================================================
    // SBL (Samba Broadcast Link)
    // ====================================================================
    private fun startSbl(call: MethodCall, result: MethodChannel.Result) {
        val ip          = call.argument<String>("engineIp")   ?: return result.error("BAD", "engineIp missing", null)
        val port        = call.argument<Int>("enginePort")    ?: 8890
        val width       = call.argument<Int>("width")         ?: 1280
        val height      = call.argument<Int>("height")        ?: 720
        ...
        thread(name = "SblStart") {
            try {
                ...
                sblSocket = DatagramSocket()
                val address = InetAddress.getByName(ip)
                streaming.set(true)

                encodeThread = thread(name = "SblEncode") {
                    drainToSbl(address, port, width, height)
                }

                result.success(null)
                Log.i(TAG, "SBL streaming → $ip:$port ${width}x$height @${bitrate/1000}kbps $codec")
            } catch (e: Exception) {
    ...
    private fun drainToSbl(address: InetAddress, port: Int, width: Int, height: Int) {
        val info = MediaCodec.BufferInfo()
        while (streaming.get()) {
            val idx = encoder?.dequeueOutputBuffer(info, 10_000) ?: break
            when {
            ...
            // Pack SBL packet
            val packetSize = 16 + 32 + data.size
            val packetBuf = ByteBuffer.allocate(packetSize).apply {
                order(ByteOrder.LITTLE_ENDIAN)
                // SBL Common Header (16 bytes)
                put("SBL".toByteArray())
                put(1.toByte()) // version
                put(1.toByte()) // streamType = VideoColor
                put(if (isKey) 1.toByte() else 2.toByte()) // frameType
                putLong(info.presentationTimeUs) // timestamp
                putInt(32 + data.size) // payloadLen

                // SBL Video Header (32 bytes)
                putInt(0x20315641) // fourCC = "AV1 "
                putInt(width)
                putInt(height)
                putInt(30) // fpsNumerator
                putInt(1) // fpsDenominator
                putInt(0) // aspectRatio
                putInt(1) // flags
            ...
            try {
                val udpPacket = DatagramPacket(packetBuf.array(), packetBuf.limit(), address, port)
                sblSocket?.send(udpPacket)
                bytesSent.addAndGet(udpPacket.length.toLong())
            } catch (e: Exception) {
                Log.w(TAG, "SBL socket send failed: $e")
            }
